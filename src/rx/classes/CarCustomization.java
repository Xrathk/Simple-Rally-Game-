            ...
            switch (choice.getIntegerInput()) {
                ...
                case 3: // UpgradeCar
                    ...
                    if (choice2.equals("yes")) {
                        ...
                        choiceUp = InputMethods.isIntegerInputValid();
                        if (!choiceUp.isInputValid()) {
                        ...
                        if (userCar.getHorsePower() + choiceUp.getIntegerInput() > maxHP) {
                            System.out.println("You're not allowed to increase your car's horsepower by that amount!");
                            System.out.printf("Your car's maximum allowed horsepower is: %d%n",maxHP);
                            break;
                        }
                        float totalUpgradeCost = upgradeCost * choiceUp.getIntegerInput();
                        System.out.printf("Upgrading your car's horsepower by %d would cost %.2f. Would you like to finalize the upgrade?", choiceUp.getIntegerInput(), totalUpgradeCost);
                        choice2 = InputMethods.YesOrNo().toLowerCase();
                        if (choice2.equals("yes")) {
                            if (userCar.getBalanceFloat() < totalUpgradeCost) {
                                ...
                                System.out.printf("Your car's new horsepower is: %d%n", userCar.getHorsePower() + choiceUp.getIntegerInput());
                                CarLoading.UpdateCar(userCar.getId(), "HorsePower", userCar.getHorsePower() + choiceUp.getIntegerInput(), totalUpgradeCost);
                            }
                        } else if (choice2.equals("no")) {
                ...
                case 4: // Customize car
                    ...
                    // Coloring text
                    do {
                        choice3 = InputMethods.isStringInputValid();
                        choice3.setStringInput(choice3.getStringInput().toLowerCase());
                        String value = textColors.get(choice3.getStringInput());
                        if (value != null) {
                            // Valid choice, change color
                            HashMap<String, Integer> colorCodes = ColoringMethods.InitColorCodes();
                            System.out.printf("Changing text color to %s...%n",choice3.getStringInput());
                            System.out.println("Changes complete!\n");
                            CarLoading.UpdateCar(userCar.getId(),"TextColor", colorCodes.get(choice3.getStringInput()), 0);
                            coloringComplete += 1;
                        } else {
                            // Invalid choice, don't change color
                            if (choice3.getStringInput().equals("no change")) {
                    ...
                    do {
                        choice3 = InputMethods.isStringInputValid();
                        choice3.setStringInput(choice3.getStringInput().toLowerCase());
                        String value = bgColors.get(choice3.getStringInput());
                        if (value != null) {
                            // Valid choice, change color
                            HashMap<String, Integer> colorCodes = ColoringMethods.InitColorCodes();
                            System.out.printf("Changing background color to %s...%n",choice3.getStringInput());
                            System.out.println("Changes complete!\n");
                            CarLoading.UpdateCar(userCar.getId(),"BgColor", colorCodes.get(choice3.getStringInput()), 0);
                            coloringComplete += 1;
                        } else {
                            // Invalid choice, don't change color
                            if (choice3.getStringInput().equals("no change")) {
